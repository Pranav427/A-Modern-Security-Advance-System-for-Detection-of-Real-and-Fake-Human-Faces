...
def render_prediction_result(
    ...
    st.markdown(f"""
        <div class="result-card {card_style}">
            <div class="result-label">Predicted Authenticity</div>
            <div class="result-value {text_style}">{verdict}</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Technical Model Details & Boundaries"):
        ...
        st.warning(
            "Forensic Boundary: This model is trained specifically on StyleGAN v1/v2 artifact signatures. "
            "It will not generalize to modern Diffusion (Midjourney/Stable Diffusion/Flux) or commercial face-swap fakes."
        )
...
def render_prediction_page(model) -> None:
    ...
    with info_cols[2]:
        with st.container(border=True):
            ...
            st.write("Optimized for StyleGAN artifacts. It will not generalize to modern diffusion models (Stable Diffusion/Midjourney) or active face-swappers.")
            st.caption("Scope: Academic research & forensic boundary demonstration.")
